...
UserManager::UserManager() : userIndex("userIndex.dat", 120), userBufferPool(80, userDatabase)
{ 
...
std::string UserManager::queryUser(const std::string &cur_User, const std::string &username_)
{
    ...
    auto target_pos = userIndex.visit(UserName{username_});
    if (target_pos.empty())
        return "-1"; // 用户不存在
    User curUser;
    auto cur_pos = userIndex.visit(UserName{cur_User});
...
{
    std::string cur_User, username_, password_, name_, mailAddr_, privilege_str;
